...
- Drag-and-drop редактор расстановки виджетов с сайдбаром профилей
- Масштабирование виджетов (колесом мыши в редакторе)
- Скрытие/показ отдельных виджетов (глаз в редакторе)
...
#### Профили расстановки
- **Сайдбар профилей** — слева от поля редактора
- **Сохранение/загрузка** — именованные профили в localStorage
- **Удаление** — кнопка удаления с подтверждением
- **Дефолтный профиль** — автосоздаётся при сбросе расстановки
- **Список профилей** — кликабельные элементы с подсветкой активного
...
│   ├── LayoutEditor.jsx         # Drag-and-drop редактор виджетов + сайдбар профилей
...
│   ├── layoutDefaults.js        # Конфигурация виджетов + профили (localStorage)
...
backups/                         # Бэкапы (git bundle + source zip)
.agents/skills/                  # Локальные скилы CodeWhale
```
...
| Frontend | React 19 |
| Сборка | Vite 6 |
| Стили | CSS (переменные, ~2040 строк) |
