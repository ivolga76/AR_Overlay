              ...
              <div key={p.id} className="dark-panel p-3 flex items-center gap-3 text-sm">
                ...
                {editPart === p.id ? (
                  <div className="flex-1 flex flex-wrap items-center gap-2">
                    <input value={editForm.name || p.name} onChange={e => setEditForm({...editForm, name: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-32" placeholder="Имя" />
                    <input value={editForm.embark_id || p.embark_id || ''} onChange={e => setEditForm({...editForm, embark_id: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-44" placeholder="Embark ID" />
                    <input value={editForm.discord_role || p.discord_role || ''} onChange={e => setEditForm({...editForm, discord_role: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-36" placeholder="Discord" />
                    <input value={editForm.amplifier || p.amplifier || ''} onChange={e => setEditForm({...editForm, amplifier: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-28" placeholder="Amplifier" />
                    <input value={editForm.shield || p.shield || ''} onChange={e => setEditForm({...editForm, shield: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-28" placeholder="Shield" />
        ...
        <div className="flex flex-col gap-1">
          {complications.map((c: any) => (
            <div key={c.id} className="dark-panel p-3 flex items-center gap-3 text-sm">
              {editComp === c.id ? (
                <>
                  <input value={editCompText} onChange={e => setEditCompText(e.target.value)} className="flex-1 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" placeholder="Текст усложнения" />
          ...
          <div className="dark-panel p-3 flex items-center gap-2 text-sm">
            <input value={newComp} onChange={e => setNewComp(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddComp()} className="flex-1 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" placeholder="Новое усложнение..." />
        ...
        <div className="flex flex-col gap-1">
          {bonusTasks.map((bt: any) => (
            <div key={bt.id} className="dark-panel p-3 flex items-center gap-3 text-sm">
              {editBt === bt.id ? (
                <>
                  <input value={editBtFields.text || bt.text} onChange={e => setEditBtFields({...editBtFields, text: e.target.value})} className="flex-1 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" placeholder="Текст задания" />
                  <input type="number" value={editBtFields.points ?? bt.points} onChange={e => setEditBtFields({...editBtFields, points: +e.target.value})} className="w-16 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" />
          ...
          {/* Add new */}
          <div className="dark-panel p-3 flex items-center gap-2 text-sm">
            <input value={newBt} onChange={e => setNewBt(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddBt()} className="flex-1 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" placeholder="Новое бонусное задание..." />
            <input type="number" value={newBtPoints} onChange={e => setNewBtPoints(+e.target.value)} className="w-16 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" min={1} />
        ...
        <div className="flex flex-col gap-1">
          {rounds.map((r: any) => (
            <div key={r.id} className="dark-panel p-3 flex items-center gap-3 text-sm">
              ...
              {editRound === r.id ? (
                <div className="flex-1 flex flex-wrap items-center gap-2">
                  <input type="number" value={editRoundForm.points_earned ?? r.points_earned} onChange={e => setEditRoundForm({...editRoundForm, points_earned: +e.target.value})} className="w-16 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" />
                  <input value={editRoundForm.map_name || r.map_name || ''} onChange={e => setEditRoundForm({...editRoundForm, map_name: e.target.value})} className="bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm w-32" placeholder="Карта" />
                  <input type="number" value={editRoundForm.deaths ?? r.deaths ?? 0} onChange={e => setEditRoundForm({...editRoundForm, deaths: +e.target.value})} className="w-12 bg-bg-primary border border-[rgba(234, 224, 205,0.2)] rounded px-2 py-1 text-sm" placeholder="Смерти" />
